...
class DeployableWingedDart(Vehicle):
    def __init__(self, name, tail_chord=0.3, tail_span=0.3,wing_span=1.6, wing_chord=0.5,logger=None):
        super().__init__(name=name, logger=logger)
        self.tail_deflection_roll = 0
        self.tail_deflection_pitch = 0
        self.tail_deflection_yaw = 0
        ...
        self.wing_anchor_point = np.array([0.0, 0.0, 0.0])
        self.wing_sweep_angle = np.deg2rad(90)
        self.wing_sweep_rate = 3.0 #rad/sec
        self.wing_aoi = np.deg2rad(2.0)
        ...
        self.wing_1 = ThinPlate(mass=0.1 * scale_mass, span=self.wing_span, chord=self.wing_chord, thickness=0.1,
                               position=np.array([self.wing_anchor_point[0] - np.sin(self.wing_sweep_angle) * self.wing_span/2,
                                                  self.wing_anchor_point[1] + np.cos(self.wing_sweep_angle) * self.wing_span/2, 0.0]),
        ...
        self.add_component(
            RectangularPrism(mass=5.0*scale_mass, length=0.5, width=0.5, height=0.5, position=np.array([0.25, 0.0, 0.0]),
                             angle=np.array([0, 0, 0])))
        self.add_component(
            RectangularPrism(mass=0.5*scale_mass, length=2, width=0.5, height=0.5, position=np.array([-1.0, 0.0, 0.0]),
                             angle=np.array([0, 0, 0])))
